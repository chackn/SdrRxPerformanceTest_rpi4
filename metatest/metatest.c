...
int main(int argc, const char *argv[])
{
        ...
        int stat;
	printf("Sample Rate=%0.1f(Msps)\n", PARAM_SAMPRATE/1000000.0);
        bladerf_open(&dev, "");
	...
	bladerf_enable_module(dev, BLADERF_CHANNEL_RX(0), true);
        ...
        {
		int ecode = bladerf_sync_rx(dev, samples, PARAM_NUMSAMPLES, &meta, timeout_ms);
		if (ecode)
			printf("BladeRF Error:%d", ecode);
